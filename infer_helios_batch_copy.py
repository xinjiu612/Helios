...
def parse_args():
    ...
    parser.add_argument(
        ...
        dest="add_noise_to_image_latents",
        action="store_true",
        help="Enable noise injection for image latents in i2v.",
    )
    parser.add_argument(
        "--no_add_noise_to_image_latents",
        dest="add_noise_to_image_latents",
        action="store_false",
        help="Disable noise injection for image latents in i2v.",
    )
    parser.set_defaults(add_noise_to_image_latents=True)
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory containing subfolders with images and prompt txt files.",
    )
...
def main():
    ...
    if args.sample_type == "t2v" and args.prompt is None:
        ...
        if args.interactive_prompt_csv_path is not None and args.use_interpolate_prompt:
            # Note: Helios/infer_helios_batch.py logic for reading prompt from file if interactive_prompt_csv_path is set
            # But the logic seems to use args.prompt as a path in that case.
            # Keeping the Helios/infer_helios_batch.py logic.
    ...
    if args.enable_low_vram_mode:
        pipe.enable_group_offload(
            ...
            num_blocks_per_group=int(args.num_blocks_per_group) if args.group_offloading_type == "block_level" else None,
            use_stream=True,
            record_stream=True,
        )
    else:
    ...
    if args.debug_mode:
        ...
        while True:
            ...
            with torch.no_grad():
                output = pipe(
                    ...
                    add_noise_to_image_latents=args.add_noise_to_image_latents,
        ...
        # Find all images and their corresponding txt prompts in subfolders
        task_items = []
        valid_extensions = (".jpg", ".jpeg", ".png", ".bmp", ".webp")
        for root, dirs, files in os.walk(args.task_dir):
            for file in files:
                if file.lower().endswith(valid_extensions):
                    img_path = os.path.join(root, file)
                    # Look for a .txt file with the same base name
                    txt_path = os.path.splitext(img_path)[0] + ".txt"
                    if os.path.exists(txt_path):
                        with open(txt_path, "r") as f:
                            prompt_text = f.read().strip()
                        task_items.append({"image": img_path, "prompt": prompt_text, "rel_path": os.path.relpath(img_path, args.task_dir)})

        if not args.enable_parallelism:
            task_items = task_items[rank::world_size]

        for item in tqdm(task_items, desc="Processing task_dir"):
            # Create subfolder structure in output_folder
            rel_dir = os.path.dirname(item["rel_path"])
            file_name = os.path.splitext(os.path.basename(item["rel_path"]))[0]
            target_sub_folder = os.path.join(args.output_folder, rel_dir)
            os.makedirs(target_sub_folder, exist_ok=True)
            output_path = os.path.join(target_sub_folder, f"{file_name}.mp4")
            ...
            with torch.no_grad():
                try:
                    output = pipe(
                        ...
                        zero_steps=args.zero_steps,
                        # i2v
                        image=load_image(item["image"]).resize((args.width, args.height)),
                        add_noise_to_image_latents=args.add_noise_to_image_latents,
                        image_noise_sigma_min=args.image_noise_sigma_min,
                        image_noise_sigma_max=args.image_noise_sigma_max,
                        # t2v
