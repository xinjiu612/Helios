...
class BucketedFeatureDataset(Dataset):
    ...
    def prepare_stage1_latent(self, vae_latent, idx, base_vae_latent=None, image_latent=None, fake_image_latent=None):
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            try:
                base_feature_data = None
                base_vae_latent = None
                source_image_latent = None
                source_fake_image_latent = None
                if (anchor_h, anchor_w) in [
                    ...
                    base_feature_data = torch.load(base_file_path, map_location="cpu", weights_only=False)
                    base_vae_latent = base_feature_data["vae_latent"]
                    source_image_latent = self._get_optional_feature_latent(base_feature_data, "image_latents")
                    source_fake_image_latent = self._get_optional_feature_latent(
                        base_feature_data, "fake_image_latents"
                    )

                feature_data = torch.load(sample_info["file_path"], map_location="cpu", weights_only=False)
                if base_feature_data is None:
                    source_image_latent = self._get_optional_feature_latent(feature_data, "image_latents")
                    source_fake_image_latent = self._get_optional_feature_latent(feature_data, "fake_image_latents")

                x0_latent, history_latent, target_latent, clean_all_vae_latent = self.prepare_stage1_latent(
                    feature_data["vae_latent"],
                    idx,
