        ...
        with torch.no_grad():
            ...
            try:
                ...
                if "stack expects a non-empty TensorList" in str(e):
                    print(f"[rank{rank}] empty TensorList at batch={idx}. Suspect files:")
                    for uttid in batch["uttid"]:
                        # uttid format: <basename>_<start>-<end>
                        base_name = uttid.rsplit("_", 1)[0]
                        guessed_video_path = os.path.join(video_folder, "videos", f"{base_name}.mp4")
                        print(f"[rank{rank}] uttid={uttid}, guessed_video_path={guessed_video_path}")
                raise
